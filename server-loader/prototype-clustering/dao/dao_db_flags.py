...
class DAO_db_flags(DAO_db):
    ...
    def getFlag(self, id):
        ...
        """
        # data = self.db_users.find({}, {"_id": 0})
        dataList = self.db_flags.find({"perspectiveId": id})
        dataList = loads(dumps(list(dataList)))
        return dataList[0]
    ...
    def updateFlag(self, flagJSON):
        key = {
            'perspectiveId': flagJSON['perspectiveId'], 'userid': flagJSON['userid']}
    ...
    def deleteFlagById(self, id):
        self.db_flags.delete_one({"perspectiveId": id})
